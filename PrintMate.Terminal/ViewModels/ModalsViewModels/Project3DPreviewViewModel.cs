...
{
    /// <summary>
    /// ViewModel для 3D просмотра проекта
    /// </summary>
    public class Project3DPreviewViewModel : BindableBase, INavigationAware
    {
        ...
        public Project3DPreviewViewModel(IEventAggregator eventAggregator, PrintService printService)
        {
        ...
        private void LoadProject(Project project)
        {
            ...
            // Debug: проверяем layer.Height для первых слоёв
            if (project.Layers != null && project.Layers.Count > 0)
            {
                Console.WriteLine($"[Project3DPreview] Layer heights check:");
                for (int i = 0; i < Math.Min(5, project.Layers.Count); i++)
                {
                    Console.WriteLine($"  Layer[{i}].Height = {project.Layers[i].Height}");
                }
                Console.WriteLine($"  GetLayerThicknessInMillimeters() = {project.GetLayerThicknessInMillimeters()}");
            }
            ...
            Console.WriteLine($"[Project3DPreview] Project loaded: {ProjectName}, {TotalLayers} layers, height: {ProjectHeight:F2}mm");
        }
        ...
        private void UpdateCurrentLayerZ()
        {
            ...
            int idx = CurrentLayerIndex - 1; // CurrentLayerIndex начинается с 1
