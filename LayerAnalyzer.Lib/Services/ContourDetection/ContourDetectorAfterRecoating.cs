...
/// <summary>
/// Детектор контуров после нанесения порошкового слоя
/// Использует полный конвейер обработки: GaussianBlur -> StructuredEdgeDetection -> GrabCut -> Sort -> Approx
/// </summary>
public class ContourDetectorAfterRecoating : IContourDetector
{
    public VectorOfVectorOfPoint GetContours(Mat image)
    {
        ...
        using var edgeDetectChannel = new Mat();

        // 1. Шумоподавление с помощью Gaussian Blur
